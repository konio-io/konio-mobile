...
import { Store, WalletConnectActions, WalletConnectState, WalletConnectStore } from "./types";
...
export const useWalletConnectStore = (store: () => Store) : WalletConnectStore => {

    const actions : WalletConnectActions = {
        init: async () => {
            const core = new Core({
        ...
        acceptRequest: async (sessionRequest: SignClientTypes.EventArguments["session_request"]) => {
            ...
            const signer = store().Koin.getters.getSigner();
            let result: any = null;
            const provider = signer.provider;
        
            switch (request.method) {
                case WC_METHODS.SIGN_MESSAGE:
                    result = await store().Koin.actions.signMessage(signer, request.params.message);
                    break;
                case WC_METHODS.SIGN_HASH:
                    result = await store().Koin.actions.signHash(signer, request.params.hash);
                    break;
                case WC_METHODS.PREPARE_TRANSACTION:
                    result = await store().Koin.actions.prepareTransaction(signer, request.params.transaction);
                    break;
                case WC_METHODS.SIGN_TRANSACTION:
                    result = await store().Koin.actions.signTransaction(signer, request.params.transaction, request.params.options?.abis);
                    break;
                case WC_METHODS.SEND_TRANSACTION:
                    result = await store().Koin.actions.sendTransaction(signer, request.params.transaction, request.params.options);
                    break;
                case WC_METHODS.SIGN_AND_SEND_TRANSACTION:
                    result = await store().Koin.actions.signAndSendTransaction(signer, request.params.transaction, request.params.options);
                    break;
                case WC_METHODS.WAIT_FOR_TRANSACTION:
                    result = await store().Koin.actions.waitForTransaction(request.params.transactionId, request.params.type, request.params.timeout);
                    store().Mana.actions.refreshMana();
                    store().Coin.actions.refreshCoins({balance: true});
                    break;
