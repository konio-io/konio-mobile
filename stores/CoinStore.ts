...
import { Coin, CoinActions, CoinGetters, CoinState, CoinStore, Store, Transaction } from "./types";
...
export const CoinStoreLoaded = hookstate(false);
...
const state = hookstate<CoinState>(
    ...
    localstored({
        key: 'coin',
        engine: AsyncStorage,
        loaded: CoinStoreLoaded
    })
);

export const useCoinStore = (store: () => Store) : CoinStore => {

    const actions : CoinActions = {
        withdrawCoin: async (args: { contractId: string, to: string, value: string, note: string }) => {
            ...
            const rcLimit = store().Setting.state.rcLimit.get();
