...
import { KoinActions, KoinGetters, KoinStore, Store } from "./types";

export const useKoinStore = (store: () => Store): KoinStore => {
    
    const actions : KoinActions = {
        signHash: async (
            signer: Signer,
            hash: string
        ): Promise<string> => {
